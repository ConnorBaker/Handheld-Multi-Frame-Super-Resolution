...
def ICA_optical_flow_iteration(
    ref_img, gradsx, gradsy, comp_img, alignment, hessian, options, params, iter_index
):
    """
    ...
    if verbose_3:
        cuda.synchronize()
        current_time = getTime(current_time, " --- Systems calculated and solved")
...
def ICA_get_new_flow(ref_img, comp_img, gradx, grady, alignment, hessian, tile_size):
    ...
    if not (0 <= patch_idy < n_patchs_y and 0 <= patch_idx < n_patchs_x):
        return
    ...
    for i in range(tile_size):
        for j in range(tile_size):
            ...
            if not (
                0 <= pixel_global_idx < imsize_x and 0 <= pixel_global_idy < imsize_y
            ):
                continue
            ...
            if not (
                0 <= new_idx < imsize_x - 1 and 0 <= new_idy < imsize_y - 1
            ):  # -1 for bicubic interpolation
                continue

            # bicubic interpolation
            normalised_pos_x, floor_x = math.modf(
                new_idx
            )  # https://www.rollpie.com/post/252
            normalised_pos_y, floor_y = math.modf(
                new_idy
