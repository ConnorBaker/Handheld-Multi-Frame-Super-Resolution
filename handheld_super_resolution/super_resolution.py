...
from .utils_image import (
    compute_grey_images,
    frame_count_denoising_gauss,
    frame_count_denoising_median,
    apply_orientation,
)
...
def main(ref_img, comp_imgs, options, params):
    ...
    compute_grey_images_ = timer(
        compute_grey_images,
        verbose_3,
        end_s="- Ref grey image estimated by {}".format(grey_method),
    )
    init_block_matching_ = timer(
        init_block_matching,
        verbose_2,
        "\nBeginning Block Matching initialisation",
        "Block Matching initialised (Total)",
    )
    init_ICA_ = timer(
        init_ICA, verbose_2, "\nBeginning ICA initialisation", "ICA initialised (Total)"
    )
    init_robustness_ = timer(
        init_robustness,
        verbose_2,
        "\nEstimating ref image local stats",
        "Local stats estimated (Total)",
    )
    compute_grey_images_ = timer(
        compute_grey_images,
        verbose_3,
        end_s="- grey images estimated by {}".format(grey_method),
    )
    align_image_block_matching_ = timer(
        align_image_block_matching,
        verbose_2,
        "Beginning block matching",
        "Block Matching (Total)",
    )
    ICA_optical_flow_ = timer(
        ICA_optical_flow,
        verbose_2,
        "\nBeginning ICA alignment",
        "Image aligned using ICA (Total)",
    )
    compute_robustness_ = timer(
        compute_robustness,
        verbose_2,
        "\nEstimating robustness",
        "Robustness estimated (Total)",
    )
    estimate_kernels_ = timer(
        estimate_kernels, verbose_2, "\nEstimating kernels", "Kernels estimated (Total)"
    )
    merge_ = timer(
    ...
    divide_ = timer(
        divide, verbose_2, end_s="\n------------------------\nImage normalized (Total)"
    )

    bayer_mode = params["mode"] == "bayer"

    debug_mode = params["debug"]
    debug_dict = {"robustness": [], "flow": []}
    ...
    reference_pyramid = init_block_matching_(
        cuda_ref_grey, options, params["block matching"]
    )
    ...
    ref_local_means, ref_local_stds = init_robustness_(
        cuda_ref_img, options, params["robustness"]
    )
    ...
    for im_id in range(n_images):
        ...
        pre_alignment = align_image_block_matching_(
            cuda_im_grey, reference_pyramid, options, params["block matching"]
        )

        #### ICA

        cuda_final_alignment = ICA_optical_flow_(
            cuda_im_grey,
            cuda_ref_grey,
            ref_gradx,
            ref_grady,
    ...
    if accumulate_r:
        ...
        merge_ref_(cuda_ref_img, cuda_kernels, num, den, options, params["merging"])
...
def process(burst_path, options=None, custom_params=None):
    ...
    if options is None:
        options = {"verbose": 0}
    currentTime, verbose_1, verbose_2 = (
        time.perf_counter(),
        options["verbose"] >= 1,
        options["verbose"] >= 2,
    )
    ...
    if "noise" not in params["merging"].keys():
    ...
    if "exif" not in params["merging"].keys():
        params["merging"]["exif"] = {}
    if "exif" not in params["robustness"].keys():
    ...
    if "tileSize" not in params["kanade"]["tuning"].keys():
        params["kanade"]["tuning"]["tileSize"] = params["block matching"]["tuning"][
            "tileSizes"
        ][0]
    if "tileSize" not in params["robustness"]["tuning"].keys():
        params["robustness"]["tuning"]["tileSize"] = params["kanade"]["tuning"][
            "tileSize"
        ]
    ...
    params["accumulated robustness denoiser"]["median"]["on"] &= params["robustness"][
        "on"
    ]
    params["accumulated robustness denoiser"]["gauss"]["on"] &= params["robustness"][
        "on"
    ]
    params["accumulated robustness denoiser"]["merge"]["on"] &= params["robustness"][
        "on"
    ]

    params["accumulated robustness denoiser"]["on"] = (
        params["accumulated robustness denoiser"]["gauss"]["on"]
        or params["accumulated robustness denoiser"]["median"]["on"]
        or params["accumulated robustness denoiser"]["merge"]["on"]
    )

    # if robustness aware denoiser is in merge mode, copy in merge params
    if params["accumulated robustness denoiser"]["merge"]["on"]:
        params["merging"]["accumulated robustness denoiser"] = params[
    ...
    handheld_output, debug_dict = main(
        ref_raw.astype(DEFAULT_NUMPY_FLOAT_TYPE),
        raw_comp.astype(DEFAULT_NUMPY_FLOAT_TYPE),
        options,
        params,
    )
    ...
    if post_frame_count_denoise:
        ...
        if median:
            handheld_output = frame_count_denoising_median(
                handheld_output, debug_dict["accumulated robustness"], median_params
            )
        if gauss:
            handheld_output = frame_count_denoising_gauss(
                handheld_output, debug_dict["accumulated robustness"], gauss_params
            )
    ...
    if post_processing_enabled:
        ...
        output_image = raw2rgb.postprocess(
            raw,
            handheld_output.copy_to_host(),
    ...
    if "Image Orientation" in tags.keys():
        ...
        ori = 1
        warnings.warns(
            "The Image Orientation EXIF tag could not be found. \
    ...
    if "accumulated robustness" in debug_dict.keys():
        debug_dict["accumulated robustness"] = apply_orientation(
            debug_dict["accumulated robustness"], ori
        )
