...
def align_image_block_matching(img, referencePyramid, options, params, debug=False):
    ...
    th_img = torch.as_tensor(img, dtype=DEFAULT_TORCH_FLOAT_TYPE, device="cuda")[
        None, None
    ]

    img_padded = F.pad(
        th_img, (paddingLeft, paddingRight, paddingTop, paddingBottom), "circular"
    )
...
def align_on_a_level(
    ...
    if previousAlignments is None:
        upsampledAlignments = cuda.to_device(
            np.zeros((h, w, 2), dtype=DEFAULT_NUMPY_FLOAT_TYPE)
        )
    else:
...
def upsample_alignments(
    ...
    upsampledAlignments = cuda.device_array(
        (n_tiles_y_new, n_tiles_x_new, 2), dtype=DEFAULT_NUMPY_FLOAT_TYPE
    )
...
def cuda_upsample_alignments(
    ...
    if not (0 <= subtile_x < n_tiles_x_new and 0 <= subtile_y < n_tiles_y_new):
        return

    # the new subtile is on the side of the image, and is not contained within a bigger old tile
    if (
        subtile_x >= repeatFactor * n_tiles_x_prev
        or subtile_y >= repeatFactor * n_tiles_y_prev
    ):
    ...
    candidate_alignment_0_shift[0] = (
        previousAlignments[prev_tile_y, prev_tile_x, 0] * upsamplingFactor
    )
    candidate_alignment_0_shift[1] = (
        previousAlignments[prev_tile_y, prev_tile_x, 1] * upsamplingFactor
    )
    ...
    candidate_alignment_vert_shift[0] = (
        previousAlignments[
            clamp(prev_tile_y + y_shift, 0, n_tiles_y_prev - 1), prev_tile_x, 0
        ]
        * upsamplingFactor
    )
    candidate_alignment_vert_shift[1] = (
        previousAlignments[
            clamp(prev_tile_y + y_shift, 0, n_tiles_y_prev - 1), prev_tile_x, 1
        ]
        * upsamplingFactor
    )

    candidate_alignment_horizontal_shift = cuda.local.array(2, DEFAULT_CUDA_FLOAT_TYPE)
    candidate_alignment_horizontal_shift[0] = (
        previousAlignments[
            prev_tile_y, clamp(prev_tile_x + x_shift, 0, n_tiles_x_prev - 1), 0
        ]
        * upsamplingFactor
    )
    candidate_alignment_horizontal_shift[1] = (
        previousAlignments[
            prev_tile_y, clamp(prev_tile_x + x_shift, 0, n_tiles_x_prev - 1), 1
        ]
        * upsamplingFactor
    )
    ...
    for i in range(tileSize):
        for j in range(tileSize):
            ...
            if 0 <= new_idx < w and 0 <= new_idy < h:
...
def local_search(
    ...
    if distance == "L1":
        ...
        cuda_L2_local_search[blockspergrid, threadsperblock](
            referencePyramidLevel,
            alternatePyramidLevel,
            tileSize,
            searchRadius,
...
def cuda_L1_local_search(
    ...
    if not (0 <= tile_y < n_patchs_y and 0 <= tile_x < n_patchs_x):
        return
